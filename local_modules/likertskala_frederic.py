...
# =============================================================================
        ...
        #Number of Items for each screen and respective column in input file
        itemtext = df.loc[0:self.itemzahl,"Items"] 
        ...
        self.scale = list()
        self.highlightboxes = list()
        self.scaletext = scaletext
        self.rating = list()
        self.instruction = visual.TextStim(win, units = "pix", text = instruction ,
                                           height = textsize, wrapWidth = 1000,
                                           color=instruction_color, bold=True,
                                           pos=instruction_pos, alignHoriz = "left")
        #self.scaletext = visual.TextStim(win, units = "pix", text = scaletext,
        # height = scaletext_height, ori=scaletext_ori, color=(-1,-1,-1),bold=True, pos=scaletext_pos)
        self.button = [visual.Rect(win, units = "pix",width = button_size[0], 
                                   height = button_size[1], pos = button_pos,
                                   lineColor="black", fillColor = [0.5,0.5,0.5]),
                       visual.TextStim(win, units = "pix", text = "Weiter",
                                       height = 20, color=(-1,-1,-1),bold=True,
                                       pos=button_pos)]
        self.mouse = mouse
        self.win = win
        
        #Frederic#
        self.please = [visual.Rect(win, units = "pix",width = 400,
                                   height = 100, pos = (0,0), lineColor="black",
                                   fillColor = "white"),
                       visual.TextStim(win, units = "pix", text = "Bitte alle Items beantworten", 
                                       height = 20, color=(-1,-1,-1),bold=True, pos=(0,0))]
        # self.boundlow = visual.Rect(win, width= 1200,
        #                             height= (((win.size[1]/2) + button_pos[1])) + button_size[0],
        #                             color ='grey',
        #                             pos=(0, (button_pos[1]-((win.size[1]/2) + button_pos[1]) / 2))  )
        # self.boundup = visual.Rect(win, width= 1200, height= ((win.size[1]/2) - y_start),
        #                            color ='grey', pos=(0, (y_start + ((win.size[1]/2) - y_start +40)/2))  )
        #####
        for j, item in enumerate(itemtext):
            self.box.append(list())
            self.rating.append(list())
            self.highlightboxes.append(visual.Rect(win, units = "pix",
                                                   width = x_scale_dist + len(scaleunit)*box_dist,
                                                   height = item_dist, pos = (0,y_start-j*item_dist), 
                                                   lineColor=None, fillColor = "darkgrey"))
