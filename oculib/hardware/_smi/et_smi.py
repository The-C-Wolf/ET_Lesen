...
import os
import time
import thread  # todo: umstellen auf threading (fuer polling mode) 
...
try:
    import iViewXAPI  # iViewX library RED-m Eyetracker
except:
    iViewXAPI = None
...
class SmiEyeTracker(EyeTracker):
    ...
    def open(self):
        ...
        res = iViewXAPI.iViewXAPI.iV_SetLogger(ct.c_int(1), ct.c_char_p("tracker.log"))

        #res = iViewXAPI.iV_Connect(ct.c_char_p('160.45.120.146'), ct.c_int(4444), ct.c_char_p('160.45.120.194'), ct.c_int(5555))
        res = iViewXAPI.iViewXAPI.iV_Connect(ct.c_char_p('127.0.0.1'), ct.c_int(4444), ct.c_char_p('127.0.0.1'), ct.c_int(5555))
        if res == 1:
            pass  # ok
